...
class Image {
     ...
     * @return string
     */
	/* static function buildImageVerify($length=5, $mode=1, $type='png', $width=100, $height=40, $verifyName='verify') {
        ...
        imagerectangle($im,0,0,119,39,$borderColor);//图形长宽 

        for($i=2;$i<118;$i++){ 
            $line_color = imagecolorallocate($im,rand(200,255),rand(200,255),rand(200,255)); 
            imageline($im,$i,2,$i,39,$line_color); 
        }
        $pic=$im;
        $fontfiles = THINK_PATH . '../Public/Font/CORBELZ.TTF';
        $font_size=10;
    
        $Str[1] = "abcdefghijklmnopqrstuvwxyz123456789";
         
        $imstr[0]["s"] = $Str[1][rand(0,34)]; 
        $imstr[0]["x"] = rand(2,5); 
       // $imstr[0]["y"] = rand(1,2); 

        $imstr[1]["s"] = $Str[1][rand(0,34)]; 

        $imstr[1]["x"] = $imstr[0]["x"]+rand(10,22)-1+rand(0,1); 


        $imstr[2]["s"] = $Str[1][rand(0,34)]; 

        $imstr[2]["x"] = $imstr[1]["x"]+rand(10,22)-1+rand(0,1); 

         
        $imstr[3]["s"] = $Str[1][rand(0,34)]; 
        $imstr[3]["x"] = $imstr[2]["x"]+rand(10,22)-1+rand(0,1); 


		$imstr[4]["s"] = $Str[1][rand(0,34)]; 
        $imstr[4]["x"] = $imstr[3]["x"]+rand(10,22)-1+rand(0,1); 

		$imstr[5]["s"] = $Str[1][rand(0,34)]; 
        $imstr[5]["x"] = $imstr[4]["x"]+rand(10,22)-1+rand(0,1); 
        $imstr[5]["y"] = rand(1,3);

        for ($i = 0; $i < $length; $i++) {
            $text_color = imagecolorallocate($im,rand(1,255),rand(1,255),rand(1,255)); 
            imagettftext($pic,rand(20,50),$imstr[5]["y"],$imstr[$i]["x"],rand(30,40),$text_color, $fontfiles,$randval{$i}); 
        }
	...
	/*static function buildImageVerify($img_src,$ttf) {
		$num_all="abcdefghigklmnpqrstuvwxyz123456789";
		$num="";
		for($i=0;$i<4;$i++){
			$num.=$num_all[rand(0,32)];
		}
		$im=imagecreatefromjpeg($img_src);
		session_start();
		$_SESSION['verify']=md5($num);
		for($a=0;$a<4;$a++){
			$num_cor[]=imagecolorallocate($im,rand(0,255),rand(0,255),rand(0,255));
		}
		imagettftext($im,rand(20,28),rand(0,10),rand(0,10),rand(20,49),$num_cor[0],$ttf,$num[0]);
		imagettftext($im,rand(20,28),rand(0,10),rand(33,43),rand(20,49),$num_cor[1],$ttf,$num[1]);
		imagettftext($im,rand(20,28),rand(0,10),rand(66,76),rand(20,49),$num_cor[2],$ttf,$num[2]);
		imagettftext($im,rand(20,28),rand(0,10),rand(99,109),rand(20,49),$num_cor[3],$ttf,$num[3]);
        Image::output($im, 'png');
    }*/
